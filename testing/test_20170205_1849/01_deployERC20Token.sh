...
console.log("DATA: tokenABI=" + JSON.stringify(tokenCompiled["<stdin>:TestERC20Token"].info.abiDefinition));
var tokenContract = web3.eth.contract(tokenCompiled["<stdin>:TestERC20Token"].info.abiDefinition);

// TokenA
var tokenAddressA = null;
var tokenTxA = null;
var tokenA = tokenContract.new("TokenA", "ERC20A", 16, {from: eth.accounts[1], data: tokenCompiled["<stdin>:TestERC20Token"].code, gas: 800000},
  function(e, contract) {
    if (!e) {
      if (!contract.address) {
...
var tokenTxB = null;
var tokenB = tokenContract.new("TokenB", "ERC20B", 18, {from: eth.accounts[1], data: tokenCompiled["<stdin>:TestERC20Token"].code, gas: 800000},
  function(e, contract) {
    if (!e) {
      if (!contract.address) {
...
var tokenTxC = null;
var tokenC = tokenContract.new("TokenC", "ERC20C", 18, {from: eth.accounts[1], data: tokenCompiled["<stdin>:TestERC20Token"].code, gas: 800000},
  function(e, contract) {
    if (!e) {
      if (!contract.address) {
