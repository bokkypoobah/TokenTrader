...
// Approval Events
console.log("RESULT: TokenA Approval");
var tokenAApproval = tokenA.Approval({}, { fromBlock: 0, toBlock: "latest" });
var i = 0;
tokenAApproval.watch(function (error, result) {
  console.log("RESULT: tokenAApproval " + i++ + ": " + JSON.stringify(result));
});
tokenAApproval.stopWatching();
