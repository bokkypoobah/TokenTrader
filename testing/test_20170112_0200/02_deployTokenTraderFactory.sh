...
var traderFactory = traderFactoryContract.new({from: eth.accounts[1], data: traderCompiled.TokenTraderFactory.code, gas: 3000000},
  function(e, contract) {
    if (!e) {
      if (!contract.address) {
...
var sellerFactory = sellerFactoryContract.new({from: eth.accounts[1], data: sellerCompiled.TokenSellerFactory.code, gas: 3000000},
  function(e, contract) {
    if (!e) {
      if (!contract.address) {
...
console.log("DATA: gntTokenTraderABI=" + JSON.stringify(gntTraderCompiled.TokenTrader.info.abiDefinition));
console.log("DATA: gntTokenTraderFactoryABI=" + JSON.stringify(gntTraderCompiled.TokenTraderFactory.info.abiDefinition));
var gntTraderFactoryContract = web3.eth.contract(gntTraderCompiled.TokenTraderFactory.info.abiDefinition);
var gntTraderFactoryTxId = null;
var gntTraderFactoryAddress = null;
var gntTraderFactory = gntTraderFactoryContract.new({from: eth.accounts[1], data: gntTraderCompiled.TokenTraderFactory.code, gas: 3000000},
  function(e, contract) {
    if (!e) {
      if (!contract.address) {
